...
bench-image-providers.mjs: dlapi vs comfly 图片直出图延迟基准对比脚本，读取 `.env.local` 的两组 key，用相同提示词多轮采样并输出 min/avg/p50/max/成功率，用于隔离编排层开销、验证供应商真实速度。支持 --runs / --only / --size / --prompt / --timeout 参数。
