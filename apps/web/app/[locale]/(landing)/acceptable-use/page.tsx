...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'sitePages.acceptableUse' })
...
export default async function AcceptableUsePage({
  ...
  const policyCards = ['allowed', 'restricted', 'sharing'].map((key) => ({
    eyebrow: t(`sections.rules.cards.${key}.eyebrow`),
    title: t(`sections.rules.cards.${key}.title`),
    body: t(`sections.rules.cards.${key}.body`),
    bullets: [1, 2, 3].map((index) => t(`sections.rules.cards.${key}.bullets.${index}`)),
  }))
