...
[locale]/(landing)/pricing/page.tsx        — 定价页 (PricingContent + 路由级 Clerk Provider)
...
[locale]/(auth)/sign-in/[[...sign-in]]/page.tsx  — 登录页 (AuthHeader + AuthClerkCard 显式挂载 SignIn + AuthFooter)
[locale]/(auth)/sign-up/[[...sign-up]]/page.tsx  — 注册页 (AuthHeader + AuthClerkCard 显式挂载 SignUp + AuthFooter)
[locale]/(app)/layout.tsx                   — 应用布局 (路由级 Clerk Provider + AppSidebar 200px + main flex-1)
...
[locale]/(editor)/layout.tsx                — 全屏编辑器布局 (路由级 Clerk Provider + 最小化容器)
```
