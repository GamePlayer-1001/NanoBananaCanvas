...
llm-node.tsx        — LLMNode 大语言模型节点 (平台执行 + 账号级 OpenAI 兼容槽位/温度/MaxTokens/SystemPrompt/流式输出)
display-node.tsx    — DisplayNode 结果展示节点 (Markdown 渲染/复制按钮)
image-gen-node.tsx  — ImageGenNode 图片生成节点 (平台执行 + 账号级 OpenAI/Google 槽位/尺寸选择/图片预览)
