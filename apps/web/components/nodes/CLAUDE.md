...
image-gen-node.tsx  — ImageGenNode 图片生成节点 (平台模式暴露 dlapi + comfly 静态图片目录并以 provider:model 复合值选中，默认链恢复为 dlapi:gpt-image-2，执行异常时由后端处理器托底到 comfly 对应兼容模型；用户模式绑定 userKeyConfigId，并按能力表动态禁用非法尺寸与比例，节点内预览固定在内容区内缩放)
