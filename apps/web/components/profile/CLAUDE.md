...
account-content.tsx: AccountContent 账户页主内容，左侧二级导航 sticky + 右侧内容面板，支持通过 query 初始化目标页签，并把账号时区下发到账户子面板
account-dashboard-tab.tsx: AccountDashboardTab 账户仪表盘页签，展示套餐摘要、积分分布、签到积分、升级入口、账本流水，并把账号时区透传给流水组件统一用户可见时间口径
