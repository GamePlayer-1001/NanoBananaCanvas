...
account-content.tsx: AccountContent 账户页主内容，左侧二级导航 sticky + 右侧内容面板，支持通过 query 初始化目标页签，并把账号时区下发到账户子面板；订阅入口当前统一收口到月度自动订阅
...
subscription-tab.tsx: SubscriptionTab 订阅页签，当前只承接月度自动订阅的权益展示与真实 Stripe 结账动作，并把套餐卖点收口为稳定积分补给与交付节奏
