...
landing-sections.tsx — Landing 首页全宽内容区集合 (标题切换式 Features 图文展示 + 四卡 Pricing 预览 + Testimonials + FAQ；首页当前顺序为 Features → Pricing → Testimonials → 模型生态区 → FAQ，FAQ 保持简洁居中标题样式，Testimonials 移除远程头像，Pricing 使用 Stripe 月付价格，并把 CTA 收口为“Free 走正常入口、付费卡未登录先进登录并回到账户订阅页、已登录直进对应 Stripe Checkout”；Features 桌面区使用 `passive: false` 的原生 wheel 监听切换内容，避免浏览器对 React 被动滚轮事件的告警)
