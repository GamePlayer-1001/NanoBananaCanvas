...
landing-sections.tsx — Landing 首页全宽内容区集合 (标题切换式 Features 图文展示 + 四卡 Pricing 预览 + Testimonials + FAQ；首页当前顺序为 Features → Pricing → Testimonials → 模型生态区 → FAQ，FAQ 保持简洁居中标题样式，Testimonials 移除远程头像，Pricing 优先读取 Stripe 月付价格并在缺失时退回说明文案，导航中的“功能/模型”统一回落为首页锚点)
