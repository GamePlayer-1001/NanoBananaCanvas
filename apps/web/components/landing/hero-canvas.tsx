 ...
 * [OUTPUT]: 对外提供 HeroCanvas 可拖拽图像节点画布
 * [POS]: landing Hero 的交互画布层，被 hero-section.tsx 消费，负责六节点因果链与连线动画
...
function NodeBackdrop({ tone }: { tone: VisualTone }) {
  ...
  return (
    <div className="absolute inset-0" style={{ backgroundImage: surfaces[tone] }}>
      {tone !== 'landscape' ? (
...
export function HeroCanvas() {
  ...
  useEffect(() => {
    function updateScale() {
      ...
      setScale(clamp(canvasRef.current.clientWidth / CANVAS_WIDTH, 0.42, 1))
    }
  ...
  function handlePointerMove(event: React.PointerEvent<HTMLDivElement>) {
    ...
    setNodes((currentNodes) =>
      currentNodes.map((node) =>
        node.id === dragStateRef.current?.id
          ? {
              ...node,
