...
explore-content.tsx: ExploreContent 客户端容器，组合较宽留白主容器、固定高度且由图片原始比例驱动卡片宽度的纯图片 Banner、圆点叠放在轮播画面上、两行分类/排序工具区、无尽下拉与按 id 去重兜底；主类型默认走“全部”，子分类优先按真实分类/节点映射，缺省再兜底关键词筛选
