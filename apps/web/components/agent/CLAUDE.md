...
agent-header.tsx              — 面板头部，展示模式、上下文摘要与轻量操作入口
agent-conversation.tsx        — 对话滚动区，串联用户消息、过程消息、提案卡片与确认卡片
agent-message-item.tsx        — 基础消息渲染器，按 role 渲染用户/助手/诊断等通用消息
agent-process-message.tsx     — 过程消息组件，承载“正在理解 / 正在搭建 / 已同步”等轻反馈
agent-proposal-card.tsx       — 工作流提案卡片，展示计划摘要、改动项与确认需求
agent-prompt-compare-card.tsx — Prompt 对比卡片，展示原始意图 / 画面提案 / 执行提示词
