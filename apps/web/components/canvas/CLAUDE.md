...
```
canvas.tsx              — Canvas 主画布组件 (ReactFlow 包裹，内置 dot Background 替代旧粒子层，右键菜单，拖线到空白处按端口类型筛选有效节点并自动补默认连线，辅助线，顶部/底部工具栏，本地草稿自动保存 + 离场云端兜底，快捷键与左下角前三次淡色提示，并在运行态统一禁删节点/连线)
