...
editor-particle-field.tsx — EditorParticleField 编辑器交互粒子背景层 (跟随视口平移/缩放的自绘点阵，鼠标邻域浮起、加粗、变大并在离开后缓动恢复；通过 useStoreApi.subscribe 订阅 transform 写 ref，避免每帧 React re-render；点阵基线半径与 alpha 已加重，肉眼清晰可见)
