...
editor-particle-field.tsx — EditorParticleField 编辑器交互粒子背景层 (跟随视口平移/缩放的自绘点阵，间距与半径以世界坐标为基准随 zoom 整体收放，鼠标邻域浮起、加粗、变大并在离开后缓动恢复；通过 useStoreApi.subscribe 订阅 transform 写 ref，避免每帧 React re-render；常态点阵浅色不喧宾夺主，命中鼠标后才显著加重)
