...
filter-node-entry-groups.ts — filterNodeEntryGroupsByPort() 按当前拖线端口类型筛选可创建节点 (强匹配优先 + any 兜底)
filter-node-entry-groups.test.ts — filterNodeEntryGroupsByPort() 回归测试 (含 any 兜底展示)
