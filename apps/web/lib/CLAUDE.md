...
platform-models.ts — 平台模型目录语义层 (/api/ai/models 单一真相源的共享类型、Provider 分组、图标展示元数据与 Agent 平台模型预设；图片平台目录现以 dlapi:gpt-image-2 为默认首项并保留 comfly 备选，下拉选中态使用 provider:model 复合值)
