...
- `service.ts`: 核心服务层，6 大函数 (checkConcurrency/submitTask/checkTask/cancelTask/listTasks/deleteTasks)，平台模式显式读 provider/model 并接回 `freeze/confirm/refund`，user_key 模式显式读 capability/configId 且不再写平台 credits 语义；同步媒体 provider 在 submit 阶段即落 R2 + completed 入库，同时对持久化输入做 data URL 清洗，避免 D1 被超大 payload 撑爆
- `service.test.ts`: 服务层回归测试，验证平台前置失败会收敛为 TaskError，而不是漏成 UNKNOWN 500，并覆盖同步输出落库 / data URL 清洗
...
- **同步直落盘**: 图片/TTS 这类同步 provider 不再把 data URL 塞进 `external_task_id`，而是在 submit 阶段直接转存 R2，只把短 URL / 元数据落进 D1
