...
- `service.ts`: 核心服务层，8 大函数 (checkConcurrency/submitTask/processDeferredTask/processQueuedTask/checkTask/cancelTask/listTasks/deleteTasks)，平台模式显式读 provider/model 并接回 `freeze/confirm/refund`，user_key 模式显式读 capability/configId 且不再写平台 credits 语义；图片任务现为“submit 先落 pending + R2 持久化执行快照 + Cloudflare Queue 投递 + Worker 真后台执行 + 完成后回写 D1/R2”，并额外在 `runDeferredTask` 做原子认领、在 `checkTask` 对卡住的 `pending + no external_task_id` 图片任务做轮询自愈补触发，避免 Consumer 缺席时一直空转到超时；持久化输入继续做 data URL 清洗，避免 D1 被超大 payload 撑爆
- `service.test.ts`: 服务层回归测试，验证平台前置失败会收敛为 TaskError，而不是漏成 UNKNOWN 500，并覆盖图片任务排队返回 / 后台完成回写 / data URL 清洗 / Queue 消费者从 R2+D1 重建执行上下文 / 轮询侧自愈补触发
...
- **执行去重 + 自愈补启动**: `runDeferredTask()` 先原子认领 `pending + no external_task_id` 任务，挡住重复 Queue/重复补触发；若生产 Consumer 缺席，`checkTask()` 会在轮询期间补触发一次后台执行，避免图片任务白白耗尽 5 分钟超时窗
