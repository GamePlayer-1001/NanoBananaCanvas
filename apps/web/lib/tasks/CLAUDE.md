...
- `service.ts`: 核心服务层，5 大函数 (checkConcurrency/submitTask/checkTask/cancelTask/listTasks)，平台模式显式读 provider/model，user_key 模式显式读 capability/configId，并在任务完成时把媒体输出统一持久化到 `outputs/{userId}/{taskId}.{ext}`
