...
- `image-gen.ts`: ImageGenProcessor（平台图片供应商处理器，支持 OpenAI/OpenRouter 参考图输入链路、Google Imagen 文生图、DLAPI 异步出图与 `dlapi -> comfly` 托底，并在未接通参考图协议的供应商上显式报错，禁止静默吞掉 `image input`）
- `image-gen.test.ts`: ImageGenProcessor 回归测试（OpenAI 兼容 url/base64、OpenRouter/兼容层参考图透传、DLAPI 异步 submit/check、Comfly fallback、未支持参考图时快速失败）
...
- **参考图语义真实化**: `imageUrl` 只有在处理器已接通供应商参考图协议时才会发给上游；当前 OpenAI/OpenRouter 兼容链路已透传，DLAPI/Gemini 未接通时会直接报错，不再假装支持
