...
## 十一、本轮体验收口（2026-04-30）

### 已完成

- [x] 将固定右侧重面板收口为右下角悬浮 Agent 卡片
- [x] 支持折叠 / 展开 / 拖动 / 拖宽
- [x] 将首屏文案改为轻引导 Hero 与 4 个示例问题
- [x] 输入区内嵌模型下拉与平台模式 / API Key 模式切换
- [x] 创建类请求改为优先自动落工作流，不再统一先卡提案审批
- [x] Prompt confirmation 改为通过聊天肯定语句继续，而不是系统级确认按钮
- [x] 将 prompt compare card 收口为轻量展示与“聊天确认提示”
- [x] 完成本轮静态验证：`pnpm --dir apps/web lint`、`pnpm --dir apps/web exec tsc --noEmit`

### 仍需后续观察

- [ ] 悬浮卡片的实际拖拽与拖宽手感仍需浏览器手测
- [ ] 模型 / 模式切换当前已完成 UI 收口，但是否需要真正接入 Agent 后端参数，还要看下一轮产品决策
