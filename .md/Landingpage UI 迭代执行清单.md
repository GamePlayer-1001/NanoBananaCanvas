...
- [x] LUI-101 审核 `BrandMark` 在导航、Hero、Footer、Auth、Sidebar 的使用位置，禁止局部硬编码品牌字标。
...
- [x] LUI-105 检查深色留白，改为满屏铺展背景，不再让黑色大面积空着。
...
- [x] LUI-801 没有真实评价时，Testimonials 改为使用场景证明卡，不虚构用户头像和姓名。
- [x] LUI-802 如果已有真实反馈，需记录来源与授权状态；当前未发现可授权真实反馈，因此不展示 Testimonials。
...
- [x] LUI-1214 生产部署完成：`main` 推送至 `9e7b84f` 后触发 GitHub Actions `24830648807`，`Lint & Build` 与 `Deploy` 均通过，Cloudflare Web 与 API Worker 已完成生产发布。
