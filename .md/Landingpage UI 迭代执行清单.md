...
- [ ] LUI-102 放大主页中央、导航右上、登录页右侧品牌字标，统一使用艺术手写字体。
- [ ] LUI-103 将 Landing 主 CTA 统一为黑白按钮系统：主按钮黑底白字，次按钮白底黑字或透明描边。
- [ ] LUI-104 更新 `landing-dark` token，切换为黑白电影感、银灰玻璃反光、颗粒质感。
- [ ] LUI-105 检查深色留白，改为满屏铺展背景，不再让黑色大面积空着。

### Phase 2 - Landing 导航与页面骨架

- [ ] LUI-201 移除企业版入口。
- [ ] LUI-202 将 Features 链接指向首页 `#features` 或 `/features`。
- [ ] LUI-203 将 Pricing 链接指向 `/pricing`。
- [ ] LUI-204 将 Models 链接指向 `#models` 或 `/models`。
- [ ] LUI-205 将 Resources 改成真实菜单：Docs、Community、Contact。
- [ ] LUI-206 所有导航链接必须有真实 href，不允许保留无效 button 占位。
- [ ] LUI-207 更新 `landing-nav.tsx` L3 头部、`components/layout/CLAUDE.md` 与 i18n key。

### Phase 3 - Hero 图像节点画布

- [ ] LUI-301 将 Hero 画板容器扩大到接近全屏，桌面端建议逻辑尺寸不小于 `1440x760`。
- [ ] LUI-302 将六个文字节点替换为图像节点，表达“脸部特征 + 人物 + 融合 + 场景 + 场景融合 + 动态输出”。
- [ ] LUI-302A 节点 1 固定为“女孩脸部特征特写”，负责提供可被继承的面部特征。
- [ ] LUI-302B 节点 2 固定为“另一个女孩的人像主体”，负责提供人物基础身份。
- [ ] LUI-302C 节点 3 固定为“节点 1 + 节点 2 的融合结果”，必须一眼看出这是“拥有该面部特征的女孩”。
- [ ] LUI-302D 节点 4 固定为“风景背景图”，负责提供环境，不得退化成文字标签。
- [ ] LUI-302E 节点 5 固定为“节点 3 + 节点 4 的融合结果”，必须表达“这个女孩在这个风景中”。
- [ ] LUI-302F 节点 6 固定为“节点 5 的动态视频感输出”，必须表达“这个女孩在动态风景中行走”，而不是静态终图。
- [ ] LUI-302G 六节点叙事必须让用户看懂 `1+1=2` 的生成逻辑，不能退化成六张无因果关系的好看图片。
- [ ] LUI-303 统一 SVG 与节点逻辑坐标，修复拖动后连线消失问题。
- [ ] LUI-304 节点拖动使用边界约束，避免节点被容器裁掉。
- [ ] LUI-305 连线增加流光或 dash 动效，表达数据流。
- [ ] LUI-306 Hero 标题放大，减少描述文字，突出品牌和一句主张。
- [ ] LUI-307 Hero CTA 改为黑白高对比，并进入 `/sign-in?redirect_url=/workspace`。
- [ ] LUI-308 更新 `components/landing/CLAUDE.md`，如果拆分新组件则补全成员清单。
...
- [ ] LUI-803 FAQ 改为点击展开的手风琴。
- [ ] LUI-804 FAQ 文案与 `FAQPage` JSON-LD 共用同一数据源。
- [ ] LUI-805 FAQ 留白加大，避免所有问题挤在一个密集网格里。

### Phase 9 - CTA + Footer

- [ ] LUI-901 新建或恢复 `CtaSection`，最后一屏只负责转化。
- [ ] LUI-902 Footer 调整为 Product、Resources、Company、Legal、Social。
- [ ] LUI-903 Footer 链接必须指向真实页面或明确待建页面，不允许全部回到 `/`。
- [ ] LUI-904 社交图标避免 emoji 占位，使用 Lucide 或真实品牌图标方案。

### Phase 10 - 公开子页面树

- [ ] LUI-1001 新建 `/features` 总览页。
...
- [ ] LUI-1101 移除左侧视觉区左上品牌标识。
- [ ] LUI-1102 移除左下解释性旧需求文案。
- [ ] LUI-1103 重写左侧诗意品牌文案，保留主标题、正文、三个板块。
- [ ] LUI-1104 替换左侧视觉为“迷幻夜晚热气球升空”方向。
- [ ] LUI-1105 右侧展示并放大 `BrandMark`。
- [ ] LUI-1106 删除“输入邮箱和密码...”说明句。
- [ ] LUI-1107 优化 Clerk 卡片阴影：向下、距离略远、扩散更大。
- [ ] LUI-1108 同步 `auth` 文案、`components/auth/CLAUDE.md` 与 `(auth)/CLAUDE.md`。

### Phase 12 - 验证与回环

- [ ] LUI-1201 运行 `pnpm --filter @nano-banana/web lint`。
- [ ] LUI-1202 运行 `pnpm --filter @nano-banana/web test`。
- [ ] LUI-1203 运行 `pnpm --filter @nano-banana/web build`。
- [ ] LUI-1204 如新增/修改 i18n key，运行 `pnpm --filter @nano-banana/web i18n:check`。
- [ ] LUI-1205 如新增公开页面，检查 `sitemap.xml` 路由与 canonical。
- [ ] LUI-1206 检查移动端、桌面端、`prefers-reduced-motion`。
- [ ] LUI-1207 检查 L3 文件头、L2 CLAUDE、L1 CLAUDE 是否需要同步。
