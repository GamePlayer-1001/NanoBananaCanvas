...
| M10 | 节点语境 | 选中节点语境、节点级操作理解与局部执行打通 | [ ] |
| M11 | 上下文增强 | 更强画布摘要、资产理解、历史记忆与多提案机制落地 | [ ] |
| M12 | 共创闭环 | V3 共创能力、审计、测试、体验与文档全部收口 | [ ] |
...
- [ ] `AGENT-700` 梳理当前模板来源与模板元数据真相源
- [ ] `AGENT-701` 为模板建立 `TemplateSummary` 类型与序列化规范
- [ ] `AGENT-702` 在画布摘要器中加入“当前基于哪个模板创建”的上下文
- [ ] `AGENT-703` 支持 Agent 解释模板目标、适用场景与当前结构

### 模板改造链路

- [ ] `AGENT-710` 新增 `POST /api/agent/template-plan`
- [ ] `AGENT-711` 支持“按行业/风格/目标改模板”的结构化计划输出
- [ ] `AGENT-712` 支持模板改造前先生成用户可读提案卡
- [ ] `AGENT-713` 支持模板改造后同步更新默认 prompt、模型、输出规格
- [ ] `AGENT-714` 支持模板改造结果回写到工作流审计记录

### UI 与体验

- [ ] `AGENT-720` Quick Actions 增加模板起手建议
- [ ] `AGENT-721` Proposal Card 增加“基于模板改造”来源标识
- [ ] `AGENT-722` 会话中展示“当前模板已被改造成什么方向”

### M7 验收

- [ ] `AGENT-730` 用户可先选模板再说“改成适合服装商品的”
- [ ] `AGENT-731` Agent 可解释模板与修改方向
- [ ] `AGENT-732` 模板改造结果能安全落图并自动保存
...
- [ ] `AGENT-800` 扩展 `AgentDiagnosis`，新增成本、速度、结构冗余三类诊断维度
- [ ] `AGENT-801` 在摘要器中加入模型价格、任务耗时、失败率、重复节点线索
- [ ] `AGENT-802` 新增 `POST /api/agent/optimize`
- [ ] `AGENT-803` 支持输出“问题 -> 原因 -> 优化提案 -> 风险”的标准结构

### 优化落图

- [ ] `AGENT-810` 支持“更便宜模型替换”提案
- [ ] `AGENT-811` 支持“慢步骤拆分/并行建议”提案
- [ ] `AGENT-812` 支持“缺 Display / 缺缓存 / 缺分支汇合”结构性修复提案
- [ ] `AGENT-813` 支持“只建议不改图”与“确认后代改”两种模式

### M8 验收

- [ ] `AGENT-820` 用户可问“哪里可以更省钱”
- [ ] `AGENT-821` 用户可问“这个流程太慢了，帮我优化一下”
- [ ] `AGENT-822` 系统能给出像样的优化提案并可选择是否应用
...
- [ ] `AGENT-900` 为摘要器加入最近输出资产摘要（图片/视频/音频/文本结果）
- [ ] `AGENT-901` 抽取“最近一次成功结果”作为下一步建议输入
- [ ] `AGENT-902` 支持结果导向的 follow-up 建议生成

### 建议到落图

- [ ] `AGENT-910` Quick Actions 增加“基于结果继续”
- [ ] `AGENT-911` 支持基于图片结果补视频分支
- [ ] `AGENT-912` 支持基于生成结果补标题/正文/变体分支
- [ ] `AGENT-913` 支持把 follow-up 建议先作为轻提案展示，再决定是否落图

### M9 验收

- [ ] `AGENT-920` 图片生成完成后系统可建议下一步
- [ ] `AGENT-921` 用户确认后可自动长出新分支
- [ ] `AGENT-922` 新分支改动可撤销、可追踪、可解释
...
- [ ] `AGENT-1010` 支持“这个节点在做什么”的节点级解释
- [ ] `AGENT-1011` 支持“把这个节点改成更写实/更便宜/更快”的节点级改动
- [ ] `AGENT-1012` 支持“从这个节点开始执行”的局部执行提案
- [ ] `AGENT-1013` 支持节点级 prompt 修改后只 patch 当前节点

### M10 验收

- [ ] `AGENT-1020` 选中 `image-gen` 节点后系统理解当前节点语境
- [ ] `AGENT-1021` 用户可对选中节点发出局部修改指令
- [ ] `AGENT-1022` 节点级执行与解释结果可被聊天区理解
...
- [ ] `AGENT-1100` 扩展 `CanvasSummary`，加入 workflow goal、资产摘要、历史诊断摘要
- [ ] `AGENT-1101` 支持长文本、长 prompt、多节点配置的分级压缩
- [ ] `AGENT-1102` 支持节点簇、子链路、结果资产的语义摘要
- [ ] `AGENT-1103` 支持 recent operations timeline，帮助 Agent 理解刚刚改过什么

### 多提案与长链记忆

- [ ] `AGENT-1110` 支持一个目标生成多版提案而非单版提案
- [ ] `AGENT-1111` 支持“更保守 / 更激进 / 更省钱 / 更高质量”多方案比较
- [ ] `AGENT-1112` 增加会话级短期创作记忆，只保存本工作流内近期上下文
- [ ] `AGENT-1113` 为多提案建立统一比较卡片与选择动作

### M11 验收

- [ ] `AGENT-1120` 复杂工作流下摘要仍稳定可读
- [ ] `AGENT-1121` 用户可在多版提案中选择方向
- [ ] `AGENT-1122` Agent 能记住最近一两轮的创作上下文，不必重复问同样问题
...
- [ ] `AGENT-1200` 为 Agent 审计记录建立持久化方案
- [ ] `AGENT-1201` 记录用户原话、摘要、plan、确认、执行、结果
- [ ] `AGENT-1202` 支持重放最近一次 Agent 改图记录
- [ ] `AGENT-1203` 为关键 Agent 事件补齐埋点与分析字段

### 体验收口

- [ ] `AGENT-1210` 实现“查看改动”聚焦入口
- [ ] `AGENT-1211` 实现“回到最新提案”与“回看上次改动”入口
- [ ] `AGENT-1212` 为多提案、节点语境、结果续写统一视觉规范
- [ ] `AGENT-1213` 对低性能设备和 reduced motion 做完整降级

### 测试与验收

- [ ] `AGENT-1220` 为新增 operation 与 optimize/template API 补单测
- [ ] `AGENT-1221` 增加 E2E：模板改造链路
- [ ] `AGENT-1222` 增加 E2E：节点级修改链路
- [ ] `AGENT-1223` 增加 E2E：基于结果自动建议下一步
- [ ] `AGENT-1224` 增加 E2E：多提案选择并落图
- [ ] `AGENT-1225` 增加 E2E：优化建议并确认应用

### M12 验收

- [ ] `AGENT-1230` V2 能力全部具备稳定测试覆盖
- [ ] `AGENT-1231` V3 共创能力完成端到端闭环
- [ ] `AGENT-1232` Agent 行为具备可追踪、可回放、可审计能力
- [ ] `AGENT-1233` 体验达到公开灰度或深度内测标准
