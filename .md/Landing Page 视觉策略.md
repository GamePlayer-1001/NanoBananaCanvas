...
┌─────────────────────────────────────────────────────────────────┐
│                                                                   │
│  左侧：超大标题 + 描述 + 主按钮 + 辅助提示卡                         │
│                                                                   │
│  中央：多层环形云脑图                                              │
│    • 中心是「多模型智能中枢」紫色发光核心                           │
│    • 外圈布置 OpenAI / Google / Anthropic / Gemini / Wan 等节点   │
│    • 内圈布置 ByteDance / Kling / Qwen / MiniMax 等节点            │
│    • 节点与中心通过虚线贝塞尔路径连接，随滚动渐入                   │
│                                                                   │
│  右侧：能力总览卡                                                 │
│    • 文本生成 / 图像生成 / 视频生成 / 3D 生成 / 语音合成           │
│    • 底部展示已接入模型数与持续更新状态                             │
│                                                                   │
└─────────────────────────────────────────────────────────────────┘
```
...
| 元素             | 引擎                    | 动画                                                 | 时机         |
| ---------------- | ----------------------- | ---------------------------------------------------- | ------------ |
| 左侧文案列       | anime.js ScrollObserver | opacity 0→1 + X:-40→0                                | 进入视口 20% |
| 云脑图主画布     | anime.js                | 整体 opacity/scale 渐入 + 轻微 rotate 漂移           | Section 可见 |
| 中心核心球       | anime.js                | 呼吸式 scale + glow 脉冲                             | 持续循环     |
| 节点连线         | anime.js                | dashoffset 流动 + 路径透明度渐显                     | Section 可见 |
| Provider 节点    | Framer Motion 风格过渡  | 逐个错峰 reveal + 上下轻微 bobbing                   | 滚动触发后   |
| 右侧能力总览卡   | anime.js ScrollObserver | opacity 0→1 + X:+40→0                                | 进入视口 20% |
