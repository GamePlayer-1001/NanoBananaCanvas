...
Landing 应从“深色科技展示页”升级为“全屏创作旅程”：Hero 的按钮与标签栏按钮使用黑白高对比，页面其他区域继续保留深色科技视觉、银灰玻璃反光与颗粒质感。页面不再依赖零散卡片堆叠，而是采用全屏 Section snap：鼠标滚轮一次推进一个叙事板块，右侧以自动隐藏的节点式进度轨提示当前位置。能被一屏讲清的内容不跨屏重复，能用图像讲清的能力不再用文字节点解释。
...
| 项目      | v1.0                | v2.0                                           |
| --------- | ------------------- | ---------------------------------------------- |
| 主色      | 低饱和紫蓝强调      | 深色科技为主，Hero 控件黑白高对比，银灰与玻璃质感为辅 |
| CTA       | 品牌紫蓝按钮        | Hero 主按钮黑底白字、次按钮白底黑字或透明描边  |
...
- 不使用大面积蓝白按钮；黑白按钮范围限定在 Hero 与标签栏/胶囊按钮，不扩散成整站黑白。
- 不使用荧光色与彩虹渐变。
- 不用三层以上嵌套的特殊动画逻辑。
...
- Hero 主 CTA 与标签栏/胶囊按钮改为黑白高对比，不再使用蓝白主按钮；Landing 其他 CTA 保持深色科技体系。
- 导航不再出现无效按钮，企业版入口移除。
...
| 2026-04-23 | v2.0 | 根据 UI 迭代需求更新 Landing 为满屏板块滚动、图像节点叙事、模型动态脑图、FAQ 手风琴与公开子页面树方案，并限定黑白视觉只用于 Hero 与标签栏按钮 |
