...
具体映射：
Hero（震撼开场）→ 模型动态脑图（能力证明）→ Features（操作实感）→ Pricing 四档（决策引导）→ Testimonials（社会证明）→ FAQ（顾虑消解）→ Footer（导航收束）
```
...
| 克制的色彩 | 主色低饱和紫蓝，点缀金属银灰，高光仅用于主操作入口与重点信息  | 禁止大面积高饱和色块   |
...
| 2026-04-28 | v1.2 | 移除已下线的独立 Final CTA Section 描述，首页叙事主线收口为 Hero → 模型区 → Features → Pricing → Testimonials → FAQ → Footer，与当前真实实现对齐 |
